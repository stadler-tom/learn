1. Kurzbeschreibung
Learning Journal ist eine kleine webbasierte Anwendung, die im Rahmen eines projektorientierten Lernkurses entwickelt wird. Das Projekt dient dazu, praktische Erfahrungen mit Git, Docker, CI/CD, Spring Boot, PostgreSQL und testgetriebener Entwicklung zu sammeln.

Die Anwendung ermöglicht es, persönliche Lernziele, Aufgaben und Fortschritte zu dokumentieren. Nutzer können Lerninhalte erfassen, Aufgaben verwalten, Bearbeitungsstände setzen und nachvollziehen, welche Themen bereits bearbeitet wurden.

Der Schwerpunkt des Projekts liegt nicht auf einem möglichst umfangreichen Endprodukt, sondern auf dem Weg dorthin: Jede Funktion wird genutzt, um reale Entwicklungsprozesse zu üben, zum Beispiel Branching, Pull Requests, Tests, Containerisierung und automatisierte Builds.

2. Lernziele

git push pull und branchcheckout ist verständlich. aber was ist der genaue nutzen von git. wie repariert man git wenn etwas hakt, wie erstellt man mergerequests, was ist ein pullrequest? wie wird cherrybicking betrieben? Kurzum, ich möchte lernen wie und warum man mit git arbeitet. 

bei docker ist es gleich: 
einen docker conteiner aus dem repo zu holen und mit run laufen lassen ist einfach, aber einen conteiner selber erstellen und abhängigkeiten einbauen: zb. eine Datenbank nicht selber erstellen, sondern einen container einhängen. und so weiter. 
und was macht docker compose, wie kann ich das project in container packen? mit dem gedanken das ganze irgendwann in kubernetes zu bringen und dann zu skalieren. 
Ich möchte vieleicht eine docker zertifizierung machen und dafür muss ich docker lernen verstehen und anwenden können. Dafür soll hier der Übungsbereich viele aufgeben bereitstellen. 

CI/CD ist ein Konstrukt das mir nichts sagt. was ist CI was ist CD? warum wird es verwendet was genau ist es wo liegt der vorteil?
wie erstelle ich pipelines? wie kann ich tests in der pipeline machen? 
ich möchte lernen wie ich pipelines erstelle und welchen vorteil ich hier habe.
ich möchte in zukunft pipelines für anfordeungen die kollegen stellen erstellen können. 

die Porgrammiersprachen vor allem java script soll verbessert werden, ich mochte im Testdriven design arbeiten das kenne ich auch noch nicht. aber mit der pipeline zusammen ergibt sich hier eine große synergie. 
hier möchte ich gerne programmieren üben, und der tutor soll mir zeigen wie man sauber coded, welche sachen man trennt und welche design s man verfolgt umd wirklich sauberen code zu erstellen. 